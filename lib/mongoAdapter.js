...
function remove (getDB) {
  return function (collection, query, req, cb) {
    ...
    getDB(function (err, db) {
      ...
      db.collection(collection).deleteOne(query, cb)
    })
  }
}
  ...
  return function (collection, doc, req, cb) {
    ...
    getDB(function (err, db) {
      ...
      if (Array.isArray(doc)) {
        db.collection(collection).insertMany(doc, function (err, res) {
          if (err) {
            return cb(err)
          }
          if (res.insertedCount < 1) {
            return cb(new Error('Mongo could not insert documents'))
          }
          cb(null, res)
        }) 
      } else {
        db.collection(collection).insertOne(doc, function (err, res) {
          ...
          if (!res.insertedId) {
            return cb(new Error('Mongo could not insert document'))
          }
          cb(null, {_id: res.insertedId})
        })
      }
    })
  }
